...
execute: # for all code chunks, in rendered doc
...
```{r}
ggpairs(drought_exp_clean, # data frame
        columns = c(                   # columns to visualize
                    "sla", 
                    "total_g"), 
        upper = list(method = "pearson")) + # calculating Pearson correlation coefficient
  theme_bw() + # cleaner theme
...
**Specific leaf area (SLA) and leave dry weight are not correlated (Pearson's r = 0.033)**  

**root mass and shoot mass have a positive correlation (Pearson's r = 0.553)**
...
**It appears that Grindelia camporum has the largest biomass, and Acmispon glaber has the smallest. Plants with larger biomasses seem to have larger ranges.**
...
**Well watered treatments have higher biomass compared to drought stressed ones. Makes sense based on what we know about botany.**
...
**Visually there doen't seem to be a realtionship, There is a weak positive correlation between specific leaf area and total mass (Pearson's r = 0.453.**
...
```
Diagnostics for saturated model looks good!
...
Out of the four models we have tried, the best model is the one that includes **Model 1 with all three predictors: SLA, species, water treatment**  

**after adding model 4, using just water treatment and species is the best model.**
...
**reference for water_treatment:** **drought_stressed**  
**reference for species_name:** **Acmispon glaber**  

Interpret the model estimates for the:  

- intercept: **Mass of Acmispon glaber in drought stressed conditions**  
- SLA slope: **across drought treatment and species, with each 1 unit increase in SLA(units), we expect an increase in total mass of -0.00025 WHICH MEANS WITH THE ESTIMATE AND STANDARD ERROR THERE IS NO EFFECT OF SLA ON TOTAL MASS**  
- well watered estimate: **across species and SLA, well watered plants tend to be 0.12 +/- 0.02 g heavier than drought stressed plants**    
- Lotus scoparius estimate: **insert response here**    
- Stipa pulchra estimate: **growth compared to Acmispon glaber if other predictors are held contstant**    
...
plot(model_preds4, # model predictions
...
model4_preds_for_plotting <- model_preds4 %>% 
...
ggplot() +
  ...
  geom_point(data = model_preds4,
             aes(x = group,
                 ...
                 color = group)) +
  theme_classic() +
  facet_wrap(~x) # creating different panels for species
```
